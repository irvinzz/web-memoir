...
export class MacCertificateManager extends CertificateManager {
  ...
  async installCertificate() {
    return await new Promise<{ code: INSTALL_CERTIFICATE_CODES; error?: any }>((resolve, reject) => {
      const securityProcess = spawn("security", [
        "add-certificate",
        "-t",
        "C,",
  ...
  private compareCertificates(cert1: any, cert2: any): boolean {
    ...
    const issuer1 = cert1.issuer.attributes
      .map((a: any) => `${a.shortName}=${a.value}`)
      .join(",");
    const issuer2 = cert2.issuer.attributes
      .map((a: any) => `${a.shortName}=${a.value}`)
      .join(",");
    ...
    const valid1 =
      cert1.validity.notBefore.getTime() === cert2.validity.notBefore.getTime();
    const valid2 =
      cert1.validity.notAfter.getTime() === cert2.validity.notAfter.getTime();
